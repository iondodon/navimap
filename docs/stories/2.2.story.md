...
- [ ] Task 1: Create Route Data Model (AC: 2)

  - [ ] Create lib/models/route.dart with polyline points and metadata
  - [ ] Add List<LatLng> points field for route coordinates
  - [ ] Include distance (meters) and duration (seconds) fields
  - [ ] Add distanceKm and durationMin helper getters for display
  - [ ] Include optional geometry field for encoded polyline data

- [ ] Task 2: Implement RoutingService with OSRM Integration (AC: 1, 3, 6, 7)

  - [ ] Create lib/services/routing_service.dart class
  - [ ] Implement calculateRoute(UserLocation, Destination) main method
  - [ ] Add \_fetchRouteFromOSRM() private method with HTTPS API calls
  - [ ] Configure OSRM request: /route/v1/driving/ with proper parameters
  - [ ] Parse OSRM JSON response to Route model
  - [ ] Handle API response within 5-second timeout (NFR3)
  - [ ] Add proper error handling for OSRM API failures

- [ ] Task 3: Add GraphHopper Fallback Implementation (AC: 4, 7)

  - [ ] Add \_fetchRouteFromGraphHopper() private method
  - [ ] Configure GraphHopper API with car vehicle type
  - [ ] Implement fallback logic: OSRM fails → try GraphHopper
  - [ ] Parse GraphHopper response format to Route model
  - [ ] Handle GraphHopper API key configuration (environment variable)
  - [ ] Add timeout and error handling for GraphHopper calls

- [ ] Task 4: Integrate Route State with AppState (AC: 5)

  - [ ] Add Route? currentRoute field to AppState
  - [ ] Implement setRoute(Route) method with notifyListeners()
  - [ ] Add route loading states (isCalculatingRoute boolean)
  - [ ] Add route error states for API failures
  - [ ] Clear route when destination changes before new calculation
  - [ ] Trigger route calculation automatically when destination is set

- [ ] Task 5: Display Route Polyline on MapScreen (AC: 2)

  - [ ] Add PolylineLayer to FlutterMap for route display
  - [ ] Configure polyline appearance: blue color, appropriate width
  - [ ] Render route points as smooth polyline overlay
  - [ ] Ensure polyline visibility across different zoom levels
  - [ ] Handle polyline updates when route changes
  - [ ] Add loading indicator during route calculation

- [ ] Task 6: Route Calculation Workflow Integration (AC: 1, 5, 7)
  - [ ] Wire destination selection to automatic route calculation
  - [ ] Show loading state during API calls
  - [ ] Display calculated route immediately when received
  - [ ] Clear previous route when new destination is selected
  - [ ] Handle route calculation errors with user feedback
  - [ ] Test complete workflow: tap destination → calculate → display route
